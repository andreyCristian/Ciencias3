...
func EvaluarLenguaje(cadena string) bool{
	...
	comExp, _ := regexp.Compile(`[>]|[<]|[!=]|[=]|[=>]|[<=]$`)
    bolExp, _ := regexp.Compile(`[false]|[true]$`)

	err := false

	for _, cad := range Separar(cadena) {
		if numExp.MatchString(cad){
			fmt.Println("Valor Numerico : ", cad)
			err = true
		} else if opExp.MatchString(cad) {
			err = true
			fmt.Println("Operador Logico : ", cad)
		} else if comExp.MatchString(cad) {
			err = true
			fmt.Println("Operador Comparativo : ", cad)
		} else if bolExp.MatchString(cad) {
			err = true
			fmt.Println("Valor Logico : ", cad)
		}else {
			err = false
...
func main() {
	...
	ImprimirTabla()
	fmt.Println("\nIngrese la expresion a evaluar en preorden, separado por espacios (ex. | 2 2) ")
	sc.Scan()

	ImprimirTabla()
	fmt.Println("La entrada fue la siguiente: " , sc.Text())
	if EvaluarLenguaje(sc.Text()) {
		t1 = ConstruirArbol(Separar(sc.Text()))
	}
	RecorrerInorden(t1)
	/*ciclo := 0
	sc:= bufio.NewScanner(os.Stdin)
